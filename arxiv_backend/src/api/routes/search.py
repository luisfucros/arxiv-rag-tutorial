from api.dependencies import CurrentUser, get_search_engine_service
...
def search_papers(
    ...
    results = search_engine_service.search(
        query=payload.query, top_k=payload.top_k, filters=payload.filters, user_id=current_user.id
    )
